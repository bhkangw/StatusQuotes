...
class UserManager(models.Manager):
    def registation_validator(self, postData):
        ...
        if len(postData['alias']) < 2:
            response['errors'].append("Alias field must be at least 2 characters")
        ...
        if not postData['birthday']:
            response['errors'].append(
                "Must add birthday so we can sing to you :)")
        elif postData['birthday'] > str(datetime.date.today()):
            response['errors'].append("Birthdate must be in the past!")
